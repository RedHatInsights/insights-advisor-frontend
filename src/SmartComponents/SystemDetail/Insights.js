...
class BaseInventoryRuleList extends Component {
  state = {
  ...
  async fetchAccountSettings() {
    try {
      const settingsFetch = (
        await API.get(`${BASE_FETCH_URL}account_setting/`, {
          credentials: 'include',
        })
      ).data;
  ...
  async fetchEntityRules() {
    ...
    try {
      ...
      const reportsFetch = (
        await API.get(`${BASE_FETCH_URL}system/${entity.id}/reports/`, {
          credentials: 'include',
        })
      ).data;
    ...
    const activeRuleIndex =
      routerData && typeof routerData.params !== 'undefined'
        ? activeReports.findIndex(
            (report) => report.rule.rule_id === this.props.routerData.params.id
          )
        : -1;
                  ...
                  <div>
                    {resolution.has_playbook ? (
                      <input
                        ...
                        checked={!!selected}
                        onChange={(event) =>
                          this.onSelect(event, !selected, rule)
                        }
  ...
  onFilterChange = (param, values) => {
    ...
    const builtRows = this.buildRows(
      activeReports,
      kbaDetailsData,
      newFilters,
      rows,
  ...
  buildFilterChips = (filters) => {
    ...
    let chips =
      filters && prunedFilters.length > 0
        ? prunedFilters.map((item) => {
            ...
            return {
              category: this.capitalize(category.title),
              chips,
    ...
    this.state.searchValue.length > 0 &&
      chips.push({
        ...
        chips: [
          { name: this.state.searchValue, value: this.state.searchValue },
        ],
      });
    return chips;
  };
  ...
  render() {
    ...
    const selectedAnsibleRules = this.getSelectedItems(rows).filter(
      (r) => r.resolution && r.resolution.has_playbook
    );
    ...
    const filterConfigItems = [
      ...
      },
      {
        ...
        filterValues: {
          ...
          onChange: (event, values) =>
            this.onFilterChange(FC.total_risk.urlParam, values),
      ...
      },
      {
        ...
        filterValues: {
          ...
          onChange: (event, values) =>
            this.onFilterChange(FC.category.urlParam, values),
      ...
      },
      {
        ...
        filterValues: {
          ...
          onChange: (event, values) =>
            this.onFilterChange(FC.has_playbook.urlParam, values),
...
BaseInventoryRuleList.propTypes = {
...
const InventoryRuleList = ({ customItnl, intlProps, ...props }) => {
  const Wrapper = customItnl ? IntlProvider : Fragment;
  const InjectedRuleList = injectIntl(BaseInventoryRuleList);
  return (
    <Wrapper
      {...(intlProps && {
